"""
Interactive CLI client for the machine-law chat WebSocket.
...
Usage:
    ...
    uv run python analysis/llm_explanations/scripts/chat_client.py --bsn 403987006 --provider vlam
    uv run python analysis/llm_explanations/scripts/chat_client.py --verbose   # toont LLM guard beslissingen
...
async def chat_session(bsn: str, provider: str, no_guard: bool = False) -> None:
    client_id = f"cli_{bsn}_{uuid.uuid4().hex[:8]}"
    uri = f"{BASE_URL}/chat/ws/{client_id}"

    print(f"Verbinding maken met {uri} (BSN: {bsn}, provider: {provider})...")

    try:
        async with websockets.connect(uri, ping_interval=20, ping_timeout=120) as ws:
            # Send initial connection data
            await ws.send(json.dumps({"bsn": bsn, "provider": provider, "no_guard": no_guard}))
...
async def _send_and_receive(ws, message: str) -> None:
    ...
    while True:
        try:
            ...
            # Brief wait for any follow-up messages (chained law results)
            try:
                ...
                if follow_msg:
                    print(f"\n[Assistent vervolg]\n{follow_msg}")
                    if VERBOSE and "guard" in follow_data:
                        guard = follow_data["guard"]
                        status = "✓ in-scope" if guard.get("valid") else "✗ out-of-scope"
                        print(f"\n  [guard] {status} — {guard.get('explanation', '')}")
                    print("-" * 60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CLI client voor de machine-law chat")
    parser.add_argument("--bsn", default="403987006", help="BSN van het profiel (default: 403987006 = Roos van Leeuwen)")
    parser.add_argument("--provider", default="claude",
        choices=["claude", "vlam", "gpt-4o", "gpt-4o-mini", "llama3.1", "llama3.2", "llama3.3", "mistral", "deepseek", "gemma2"],
        help="LLM provider (default: claude)")
    parser.add_argument("--host", default="localhost:8000", help="Host:port van de webserver (default: localhost:8000)")
    parser.add_argument("--verbose", action="store_true", help="Toon LLM guard beslissingen")
    parser.add_argument("--no-guard", action="store_true", dest="no_guard", help="Schakel de LLM guard uit")
    args = parser.parse_args()

    BASE_URL = f"ws://{args.host}"
    VERBOSE = args.verbose

    asyncio.run(chat_session(args.bsn, args.provider, args.no_guard))
